...
\section{Синтез цифровых САУ } \label{subsec:ch4/sect4/sub3}
...
\begin{itemize}
	...
	\item дискретность преобразователя сигналов  СКВТ в качестве датчика угла (ДУ), для 14 разрядов дискрета составляет ($\varDelta_{\textit{ду}} = 1.3183$ угл. мин), принципиальная схема датчика показана на рисуноке \ref{fig:SKVT2}, в соответствии с \cite[]{1310HM025} для 14 разрядного преобразования датчик можно представить в виде дискретного звена со следующей ПФ:
	\begin{equation}
	\label{eq:p4:SKVT}
	\begin{alignedat}{2}
	...
	\item частота широтно-импульсной модуляции (ШИМ) усилителя ($f_y~=~16000$Гц).
\end{itemize} 
...
\begin{enumerate}
	...
	\end{equation}
	где  \(  \omega _{\textit{ср}}\) - частота среза непрерывной системы,  \(  \omega _{\textit{гр}}\) - граничная частота,  \( T- \) период повторения (квантование по времени) ЦВУ.\par	
	\item Малые постоянные времени (\textit{Т\textsubscript{i} <\textsubscript{ср}\textsuperscript{-1}}) должны удовлетворять условию: \textit{Т\textsubscript{i} <}0,5\textit{Т.}\par
	\item Переход оси нуля децибел асимптотической ЛАХ непрерывной части, разомкнутой САУ проходит при отрицательном наклоне 20 \textit{дб/дек.}\par
	\item При построении ЛАХ разомкнутой САУ с ЦВУ в высокочастотной области  \(  \left(  \omega _{}> \omega _{ср} \right)  \) следует учитывать сумму малых постоянных времени ( \(  \sum _{}^{}T_{i} \) ), при этом необходимо выполнить  ещё одно дополнительное условие\par	
	\begin{equation}%\tag{419}
	\label{eq:p4:419}
	\frac{T}{2}+ \sum _{}^{}T_{i} \leq \frac{1}{ \omega _{\textit{ср}}}\frac{M}{ \left( M+1 \right)},
	\end{equation}
...
Таким образом, при выполнении условий  ЛАХ разомкнутой САУ с ЦВУ в контуре управления в низкочастотной области  \(  \left(  \omega _{}< \omega _{ср} \right)  \)  можно заменить ЛАХ линейной разомкнутой САУ в этой же области. 

\textbf{\textit{Оценим} }периоды квантования. 

Для ШИМ усилителя: \textit{Т\textsubscript{1} = (f\textsubscript{у})}\textsuperscript{-1}\textit{\textsubscript{ }}= 1/8000=0,000125с.

Для ДУ период квантования оценим из условия обеспечения измерения угла отработки двигателем минимальной угловой скорости: 

\begin{equation}
\alpha_0 = 12 \textit{град/сек}, T_2 = \frac{\varDelta_{\textit{ду}}}{\dot \alpha_0}; 
\end{equation}

\begin{equation}
	 T_{2}=\frac{5.2734 \cdot 57.3}{3438 \cdot 12}=0.0073с. 
\end{equation}

Из расчетов следует, что предельным периодом квантования ЦСАУ будет $T = 0.0073 c$. С учетом условий (\ref{eq:p4:418}),(\ref{eq:p4:419}) при $M=1.1$ оценим:\par

\begin{equation}
\label{eq:p4:x}
\begin{multlined}
\omega_{\textit{гр}} = \frac{2}{T} = \frac{2}{0.0073} = 273.97 c^{-1}, \\
...
\begin{multlined}
\frac{T}{2}+ \sum _{}^{}T_{i} \leq \frac{1}{ \omega _{\textit{ср}}}\frac{M}{ \left( M+1 \right)}, 
\rightarrow 0,00365+0.00015=
0,0038 \leq \frac{1}{ \omega _{ср}}\frac{M}{ \left( M+1 \right) ^{}}, \rightarrow    
\omega _{ср} \leq \frac{1}{0,0038}\frac{1,1}{2,1^{}}=137,845с^{-1} \rightarrow 
125с^{-1},
lg \left( 125 \right) =2,097.  
\end{multlined}
\end{equation}\par

С учетом условий (\ref{eq:p4:418}),(\ref{eq:p4:419}) получены требования к линейной части ЦСАУ.\par

На основе проведенных расчетов синтезируем частотным методом \cite{Bessekerski} корректирующее звено 417 с новыми параметрами: \par

\begin{equation}
T_{k1} = 0.05 c, T_{k2} = 0.00005 c, 
\end{equation}\par

обеспечивающие  устойчивость ЦСАУ с запасами устойчивости по амплитуде (L = 50 дБ) и фазе  =83 град) (рисунок \ref{fig:DSAU}). Для обеспечения требуемой точности наведения добротность САУ по скорости определена в пределах К\textsubscript{с1}= (3590 – 3700) с\textsuperscript{-1}, соответственно - К\textsubscript{д1}К\textsubscript{к1 }К\textsubscript{у1}= (700-1000) В/рад.\par

\begin{figure}[!ht]
	\centering
	\includegraphics[width=1.0\linewidth]{image55} 
	\caption{ЛАХ и ЛФХ разомкнутой линейной части ЦСАУ}
	\label{fig:DSAU}
\end{figure}
